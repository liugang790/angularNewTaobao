...
gulp.task('release.script', function() {
    ...
    if(config.getAppScripts().otherapps) {
        ...
        for (var i = 0; i < otherappsConfig.length; i++) {
            for (var j = 0; j < otherappsConfig[i].src.length; j++) {
                scripts.push(otherappsConfig[i].src[j]);
            }
        }
    }
    // 源码
...
gulp.task('release.injectTheme', function() {
    return gulp.src('./src/theme/apptheme.css')
        .pipe(gulp.dest(config.getAppDistDir() + '/theme'));
})
